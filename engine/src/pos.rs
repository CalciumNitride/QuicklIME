...
// 文節区切りの「付属語 (助詞・助動詞・接尾語) かどうか」の判定と、
...
impl FunctionalIds {
    ...
    pub fn load_from(reader: impl BufRead) -> io::Result<Self> {
        ...
        for line in reader.lines() {
            ...
            // 接尾語は「名詞,接尾,助数詞」「動詞,接尾」のように2列目が「接尾」。
            // 「名詞,接尾可能」(単独でも使える語) は別品詞なので一致させない
            let functional = pos.starts_with("助詞")
                || pos.starts_with("助動詞")
                || pos.split(',').nth(1) == Some("接尾");
...
mod tests {
    ...
    fn 助詞と助動詞と接尾語が付属語になる() {
        ...
        assert!(!ids.is_functional(6));
        assert!(!ids.is_functional(7));
    }
